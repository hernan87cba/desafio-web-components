...
function loadData(space_id, environment_id, content_model, access_token) {
  ...
  fetch(contentfulURL)
    ...
    .then((data) => {
      const portfolioTitleWhiteEl = document.querySelector(
        ".bienvenida__title-white"
      );
      portfolioTitleWhiteEl.textContent =
        data.items[0].fields.portfolioTitleWhite;

      const portfolioTitleCyanEl = document.querySelector(
        ".bienvenida__title-cyan"
      );

      portfolioTitleCyanEl.textContent =
        data.items[0].fields.portfolioTitleCyan;

      /*Busqueda de imagen del sitio*/
      const imgArray = data.includes.Asset;

      const portfolioImgEl = document.querySelector(".bienvenida__img");

      const portfolioImgTitle = "portfolio";
      const portfolioItem = imgArray.find(
        (item) => item.fields.title == portfolioImgTitle
      );
      const portfolioImgUrl = portfolioItem.fields.file.url;

      portfolioImgEl.setAttribute("src", portfolioImgUrl);
      portfolioImgEl.setAttribute("alt", portfolioImgTitle);
    })
  ...
  fetch(contentfulURL)
    ...
    .then((data) => {
      ...
      if (itemsArray != undefined && itemsArray.length > 0) {
        ...
        //Este if me va a limitar la cantidad de cards a 3 si es que hay más de 3 trabajos. Si hay menos de tres,
        ...
        if (itemsArray.length > 3) {
          ...
          for (let j = 3; j < itemsArray.length; j++) {
            ...
            containerItems++;
            //Si se llenó el contenedor o no hay más items, agrego éste al contenedor de trabajos
...
function showVerMasButton() {
  ...
  const cardsContainersEls = document.querySelectorAll(
    ".cards__container-extra"
  );

  verMasButtonEl.addEventListener("click", () => {
    verMasButtonEl.style.display = "none";
    verMenosButtonEl.style.display = "flex";
    portfolioSectionEl.classList.toggle("expanded");
    for (let i = 0; i < cardsContainersEls.length; i++) {
      cardsContainersEls[i].style.display = "flex";
    }
  });
  ...
  verMenosButtonEl.addEventListener("click", () => {
    ...
    portfolioSectionEl.classList.toggle("expanded");
    for (let i = 0; i < cardsContainersEls.length; i++) {
      cardsContainersEls[i].style.display = "none";
    }
  });
}
